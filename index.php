<?php
$conn = mysqli_connect("localhost", "", "3d*", "");

mysqli_query($conn, "CREATE TABLE IF NOT EXISTS visitors_log (
...
function getCountry($ip) {
    try {
        ...
        return isset($data['country']) ? mysqli_real_escape_string($GLOBALS['conn'], $data['country']) : 'Unknown';
    } catch (Exception $e) {
        return 'Unknown';
    }
}
...
$isUnique = isset($_COOKIE['crudgener']) ? 0 : 1;
if ($isUnique) {
    setcookie("crudgener", "1", time() + (86400 * 365), "/");
}

$res = mysqli_query($conn, "SELECT COUNT(*) as c FROM visitors_log WHERE ip = '$ip'");
$prevVisits = mysqli_fetch_assoc($res)['c'];
$nextVisitCount = $prevVisits + 1;

mysqli_query($conn, "INSERT INTO visitors_log (ip, user_agent, browser, os, device, country, is_unique, total_visits) 
VALUES ('$ip', '".addslashes($ua)."', '$browser', '$os', '$device', '$country', '$isUnique', '$nextVisitCount')");
?>

<script>
if (!localStorage.getItem("crudgener")) {
    localStorage.setItem("crudgener", "1");
}
</script>
<!DOCTYPE html>
<html lang="en">
